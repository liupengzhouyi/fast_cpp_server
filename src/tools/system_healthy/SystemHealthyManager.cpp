...
#include "SystemHealthyManager.h"
...
SystemHealthyManager::SystemHealthyManager() : running_(false) {}
SystemHealthyManager::~SystemHealthyManager() {
  Shutdown();
}

SystemHealthyManager& SystemHealthyManager::Instance() {
  static SystemHealthyManager instance;
  return instance;
}

void SystemHealthyManager::Init(int update_interval_sec) {
  interval_sec_ = update_interval_sec;
  if (running_) return;
  running_ = true;
  worker_ = std::thread(&SystemHealthyManager::WorkerLoop, this);
  MYLOG_INFO("SystemHealthyManager started with interval: %d sec", interval_sec_);
}

void SystemHealthyManager::Shutdown() {
  if (!running_) return;
  running_ = false;
  if (worker_.joinable()) worker_.join();
  MYLOG_INFO("SystemHealthyManager stopped.");
}

void SystemHealthyManager::WorkerLoop() {
  while (running_) {
...
const SystemHealthy::SystemInfo SystemHealthyManager::GetSystemInfo() {
  std::lock_guard<std::mutex> lock(mutex_);
